...
<div id="container">
	...
	<div>
						...
						{#each Object.entries($gameConfig.keymaps.get($selectedKeymap) || {}) as [key, { key: value, editable, title }]}
							<tr {title}>
								<td
									...
									onclick={() =>
										editable ? startEditing(key) : null}
									>{key}</td
								>
								<td
									class:non-editable={!editable}
								...
								>
									{#if $editingKey === key}
										<span
											>{$t(
												"common.preferences-press-any-key",
											)}...</span
										>
									{:else}
										{value}
		...
		<div class="panel">
			...
			<div>
				<label for="ghost-piece"
					>{$t("common.preferences-ghost-piece")}</label
				>
				<input
					id="ghost-piece"
					type="checkbox"
			...
			<div style="display: none;">
				...
				{#each $right_components as component, index}
					<div>
						...
						<button
							onclick={() =>
								moveComponentUp(right_components, index)}
							>Up</button
						>
						<button
							onclick={() =>
								moveComponentDown(right_components, index)}
							>Down</button
						>
					</div>
				{/each}
			...
			>-->
				...
				{#each $left_components as component, index}
					<div>
						...
						<button
							onclick={() =>
								moveComponentUp(left_components, index)}
							>Up</button
						>
						<button
							onclick={() =>
								moveComponentDown(left_components, index)}
							>Down</button
						>
					</div>
				{/each}
		...
		<div class="panel">
			...
			<select
				bind:value={$gameConfig!.language}
