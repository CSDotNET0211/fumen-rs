<script lang="ts">
	...
	import { TetrisEnv, WIDTH, HEIGHT } from "tetris";
	...
	let ghostBoard: Tetromino[] = new Array(WIDTH * HEIGHT).fill(null);
	let ghosts: boolean[] = new Array(WIDTH * HEIGHT).fill(false);
	...
	onMount(async () => {
		...
		unlistenRequestBotField = await listen<TetrisEnv>(
			"requestbotfield",
			() => {
				let clone = env!.clone();
		...
		tetrisBoardApp!.ticker.add(() => {
			...
			if (env !== null) {
				...
				if (env.current) {
					const shape =
						SHAPES[env.current.type][env.current.rotation];
					...
					for (let i = 0; i < shape.length; i++) {
						...
						board[posY * WIDTH + posX] = env.current.type;
					}
				}
				...
				if (env.current && get(gameConfig)?.ghostPiece) {
					...
					for (let i = 0; i < shape.length; i++) {
						...
						if (board[posY * WIDTH + posX] === Tetromino.Empty) {
							board[posY * WIDTH + posX] = env.current.type;
							ghosts[posY * WIDTH + posX] = true;
						}
					}
				}
