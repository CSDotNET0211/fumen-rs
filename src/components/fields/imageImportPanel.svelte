<script lang="ts">
  ...
  async function debugOutput() {
    ...
    const chunks = ImageProcessor.getBlockChunks(image.width, image.height).map(
      (chunk) => ImageProcessor.shrinkChunk(chunk)
    );
  ...
  async function saveChunkImages(
    ...
    for (let i = 0; i < chunks.length; i++) {
      const base64Chunk = ImageProcessor.createImageFromRGBA(chunks[i], size, {
        width: 10,
        height: 10,
      });
  ...
  async function applySelectedPage() {
    ...
    for (
      let i = 0;
      i < TetrisEnv.WIDTH * TetrisEnv.HEIGHT - chunkDataArray.length;
      i++
    ) {
    ...
    for (const chunkData of chunkDataArray) {
      ...
      try {
        ...
        img.src = base64Chunk;
        await img.decode();
        const prediction = await get(teachableMachineModel)!.predict(img);
    ...
    fields.update((tetris_fields: TetrisEnv[]) => {
      ...
      for (
        let i = predictions.length;
        i < TetrisEnv.WIDTH * TetrisEnv.HEIGHT;
        i++
      ) {
        field[i] = Tetromino.Empty;
      }
