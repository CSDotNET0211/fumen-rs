<script lang="ts">
	...
	import { HEIGHT, TetrisEnv, WIDTH } from "tetris";
			...
			async (event) => {
				...
				try {
					...
					await invoke("search_bot_best", {
						...
						let ghosts = Array(WIDTH * HEIGHT).fill(false);
						for (let i = 0; i < 8; i += 2) {
							let x = typedResult[i];
							let y = typedResult[i + 1];
							ghosts[x + y * WIDTH] = true;
						}

						let board = env.board;
						for (let i = 0; i < 4; i++) {
							board[
								typedResult[i * 2] + typedResult[i * 2 + 1] * 10
	...
	<div id="container">
		...
		<details id="advanced-settings" tabindex="-1">
			...
			<div class="advanced-setting">
				<label for="thinking-time"
					>{$t("common.bot-thinking-time")}</label
				>
				<input
					id="thinking-time"
					type="number"
			...
			<div class="advanced-setting">
				<label for="max-next-pieces"
					>{$t("common.bot-max-next-pieces")}</label
				>
				<input
					id="max-next-pieces"
					type="number"
