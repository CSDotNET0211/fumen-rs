<script lang="ts">
  ...
  function generateSubmenu(
    ...
    console.log("Generating submenu for path:", menuPath);
    ...
    subMenuItems.forEach((subMenuItem) => {
      switch (subMenuItem.type) {
        ...
        case MenuItemType.Normal:
          ...
          subMenuItemElement.onmouseenter = () => {
            console.log("Submenu item mouse enter", subMenuItem);
  ...
  // 「デスクトップで開く」ボタンのクリック処理
  function openInDesktop() {
    // ここでデスクトップアプリのダウンロードページ等に遷移
  ...
  function handleSubMenuMouseEnter(
    ...
    const existingSubMenu = subMenuItemElement.querySelector(".submenu");
    if (existingSubMenu) {
      existingSubMenu.remove();
      console.log("Existing submenu removed");
    }
...
<div class="panel" data-tauri-drag-region>
  ...
  {#if isWebMode}
    ...
    <button class="open-desktop-button" onclick={openInDesktop}>
      {$t("common.menu-open-in-desktop")}
    </button>
  {/if}

  <div class="play-controls">
    {#if $boardViewContent === BoardViewContentType.TetrisEdit}
      <button class="play-control-button" data-type="Play" onclick={playFumen}>
    ...
    {/if}
    <button class="play-control-button" data-type="More" onclick={moreOptions}>
