<script lang="ts">
  ...
  onMount(async () => {
    if (fumenPages == null) {
      return;
    } else if (get(fumenPages)) {
      internalFumenPages = get(fumenPages)!;
      totalPages = internalFumenPages.length;
  ...
  async function updateThumbnail() {
    if (internalFumenPages == null) return;
