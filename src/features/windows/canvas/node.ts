...
let db: Database | null = null; // MySQLのデータベース接続
...
export class TextNode extends Node {
	...
	static insertDB(x: number, y: number, size: number, text: string, color: string, backgroundColor: string): number {
		...
		console.log("TextNode created:", { id, x, y, size, text, color, backgroundColor });
	...
	static deleteDB(id: number): void {
		...
		const event = new CustomEvent("textNodeChanged", { detail: { id, action: "delete" } });
		document.dispatchEvent(event);
	}
...
export class FieldNode extends Node {
	...
	static getFromDB(id: number): TetrisEnv | null {
		if (id < 1) {
			return null;
		}
	...
	static getAllFromDB(): Array<{ id: number, jsonData: string, x: number | null, y: number | null, thumbnail: string | null }> {
		const result = db!.exec(`
	...
	static async updateDB(index: number, env: TetrisEnv) {
		const jsonData = JSON.stringify(env);
	...
	static updateCoordinatesDB(id: number, x?: number, y?: number) {
	...
	static async updateThumbnailDB(id: number) {
	...
	static async updateAllThumbnailsDB() {
